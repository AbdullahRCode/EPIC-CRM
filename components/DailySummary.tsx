...
export default function DailySummary({ ownerMode, branch: globalBranch }: DailySummaryProps) {
  const [period, setPeriod] = useState<Period>("daily");
  const [summaryBranch, setSummaryBranch] = useState<Branch | "All">(globalBranch);
  const [data, setData] = useState<DailySummaryData | null>(null);
  const [loading, setLoading] = useState(false);
  const [branchOpen, setBranchOpen] = useState(false);
  const [error, setError] = useState("");

  const effectiveBranch = ownerMode ? summaryBranch : globalBranch;
  ...
  async function generate() {
    ...
    try {
      const res = await fetch("/api/ai/summary", {
        ...
        body: JSON.stringify({ branch: effectiveBranch, period }),
      });
    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex flex-col gap-1">
          ...
          <p className="label" style={{ color: "var(--muted)" }}>
            {effectiveBranch === "All" ? "All branches" : effectiveBranch}
            {" — "}
            {period === "daily" ? "daily briefing" : "weekly briefing"}
          </p>
        </div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
            ...
            {(["daily", "weekly"] as Period[]).map((p) => (
              <button
                ...
                style={{
                  ...
                  borderRight: p === "daily" ? "1px solid var(--line)" : "none",
                }}
              >
          ...
          {/* Branch override (owner only) */}
          {ownerMode && (
            <select
              className="input-line"
              style={{ fontSize: "0.65rem", minWidth: "9rem" }}
              value={summaryBranch}
              onChange={(e) => { setSummaryBranch(e.target.value as Branch | "All"); setData(null); }}
            >
      ...
      {!data && !loading && (
        <div className="flex flex-col items-center gap-3 py-8">
          <p
            ...
            style={{ fontStyle: "italic", color: "var(--muted)", fontSize: "1rem" }}
          >
            No summary yet
          </p>
          <p className="label" style={{ color: "var(--muted)" }}>
            Generate a {period === "daily" ? "daily" : "weekly"} AI briefing for{" "}
            {effectiveBranch === "All" ? "all branches" : effectiveBranch}
          </p>
          <button onClick={generate} className="btn btn-primary">
            Generate {period === "daily" ? "today's" : "this week's"} summary
          </button>
        </div>
      )}
      ...
      {loading && (
        <div className="flex items-center gap-3 py-8 justify-center">
          <span className="label" style={{ color: "var(--muted)" }}>
            Analysing {period === "weekly" ? "week's" : "today's"} activity...
          </span>
        </div>
      )}
      ...
      {data && (
        <div className="flex flex-col gap-6">
          {/* Period summary */}
          <div>
            <p className="label mb-2">{periodLabel}</p>
          ...
          >
            <p className="label mb-2" style={{ color: "var(--vip)" }}>
              {period === "weekly" ? "Next week's outlook" : "Tomorrow's outlook"}
            </p>
