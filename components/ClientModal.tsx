...
export default function ClientModal({
  ...
  // New-entry notes field (saved directly as a visit note — no AI parsing)
  ...
  const [section, setSection] = useState<
    "identity" | "visit" | "events" | "alterations" | "order" | "followup" | "measurements"
  >("identity");
  const [notifying, setNotifying] = useState(false);

  const tags = initialClient ? deriveTags({ ...initialClient, ...form } as Client) : [];

  // ─── Visit form (edit mode only) ───────────────────────────────────────────
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (isNew && noteText.trim()) {
        // Save notes directly as a visit note — no AI parsing
        const visit: Visit = {
          id: uid(),
          date: new Date().toISOString().split("T")[0],
          reason: "Walk-in (purchased)",
        ...
        finalForm = { ...finalForm, visits: [visit] };
      }

      if (!isNew && addingVisit && newVisit.date && newVisit.reason) {
  ...
  return (
    <div className="modal-overlay" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div
        ...
        style={{ width: "min(520px, 100vw)", background: "var(--paper)" }}
      >
        {/* Header */}
        <div
        ...
        {/* ══ NEW ENTRY: simple one-step form ══════════════════════════════ */}
        {isNew && (
          <div className="flex-1 px-6 py-6 flex flex-col gap-5">
            ...
            <div style={{ borderTop: "1px solid var(--line)", paddingTop: "1.25rem" }}>
              <p className="label mb-1">
                ...
                >
                  — purchases, alterations, events, measurements, follow-ups
                </span>
              </p>
              <textarea
                className="input-line"
                placeholder="e.g. Navy 3-piece suit, $1340, wedding July 12, trousers hemmed, chest 40 waist 34, VIP."
                rows={6}
                value={noteText}
        ...
        {/* ══ EXISTING CLIENT: tab-based editing ═══════════════════════════ */}
        {!isNew && (
          <>
            <div
              ...
              {SECTIONS.map((s) => (
                <button
                  ...
                  style={{
                    ...
                    border: "none",
                    borderBottom:
                      section === s.id ? "2px solid var(--ink)" : "2px solid transparent",
                ...
                <div className="flex flex-col gap-5">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      value={form.branch}
                      onChange={(e) =>
                        setForm((f) => ({ ...f, branch: e.target.value as Branch }))
                      }
                    >
                      {BRANCHES.map((b) => (
                        <option key={b} value={b}>{b}</option>
                        ...
                        {[...form.visits]
                          ...
                          .map((v) => (
                            ...
                            >
                              ...
                              {v.notes && (
                                <p style={{ fontSize: "0.8rem", color: "var(--muted)" }}>{v.notes}</p>
                              )}
                              {v.staff && (
                    ...
                    >
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setNewVisit((v) => ({ ...v, items: e.target.value }))
                          }
                        />
                      </div>
                      <div>
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setNewVisit((v) => ({ ...v, staff: e.target.value }))
                          }
                        />
                      </div>
                      <div>
                        ...
                        <textarea
                          ...
                          onChange={(e) =>
                            setNewVisit((v) => ({ ...v, notes: e.target.value }))
                          }
                    ...
                    <>
                      ...
                      <div>
                        ...
                        <textarea
                          ...
                          onChange={(e) =>
                            setForm((f) => ({ ...f, event_note: e.target.value }))
                          }
                          style={{ resize: "vertical" }}
                        />
                      </div>
              ...
              {section === "alterations" && (
                <div className="flex flex-col gap-5">
                  <MultiSelect
                    ...
                    onChange={(v) =>
                      setForm((f) => ({ ...f, alterations: v as AlterationItem[] }))
                    }
                ...
                <div className="flex flex-col gap-5">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setForm((f) => ({ ...f, special_order: e.target.value }))
                      }
                    />
                  </div>
                  {form.special_order?.trim() && (
                    <>
                      ...
                      {form.special_order_status === "Arrived" && (
                        <div
                          className="flex flex-col gap-2 p-4"
                          style={{ background: "#1f5a3208", border: "1px solid var(--good)" }}
                        >
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-2">
            {isNew ? (
              <div className="flex gap-2 ml-auto w-full sm:w-auto">
                <button onClick={onClose} className="btn btn-ghost flex-1 sm:flex-none">
                  Cancel
                </button>
                <button
                  onClick={handleSave}
